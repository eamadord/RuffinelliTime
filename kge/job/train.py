...
class TrainingJob(Job):
    ...
    def __init__(
        ...
        self.epoch: int = 0
        self.max_time: int= self.config.get("train.max_train_time")
        self.force_time: bool = self.config.get("train.force_time")
        ...
        while True:
            ...
            if self.force_time:
                ...
                if execution_time>=self.max_time:
                    self.config.log("Maximum training time reached.")
                    self.config.log("Time elapsed: %.f"%execution_time)
                    break
            if not self.force_time and self.epoch >= self.config.get("train.max_epochs"):
            ...
            if self.epoch==1:
                total_epochs=int(self.max_time/trace_entry.get('epoch_time'))
                if trace_entry.get('epoch_time')>=self.max_time:
                    self.config.set("valid.every", 1)
                elif self.config.get("valid.every")>total_epochs:
                    self.config.set("valid.every", total_epochs)
                elif not total_epochs%self.config.get("valid.every")==0:
                    self.config.set("valid.every",math.gcd(total_epochs,self.config.get("valid.every")))
