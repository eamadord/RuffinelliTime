    ...
    def run(self):
        ...
        while trial_no < self.num_trials:
            ...
            if trial_id is None:
                ...
                # if self.force_search_time:
                #     config.set("train.force_time",True)
                #     config.set("train.start_time",start_time)
                #     config.set("train.max_train_time",self.config.get("ax_search.max_search_time"))
            ...
            # if self.force_search_time and (time.time() - start_time) >= self.max_search_time:
            #     self.config.log("Maximum search time reached.")
            #     self.config.log("Total time elapsed %.f" %(time.time()-start_time))
            #     break
